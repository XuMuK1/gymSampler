...
print("Start Test 8: Sample Trajectories From StateSampler (Parallel)")
gymEnv = gym.make("CartPole-v0")
print('DEBUG',gymEnv.reset())
state0 = np.concatenate([gymEnv.reset()[None,:] for k in np.arange(5)],axis=0)
gymEnv.reset()
...
#####DISCRETE TESTS
print('*********************TESTS IN DISCRETE ENV (TAXI)*********************')

#####Test 0: try to sample from StateSampler and check if the dimension is correct
print("Start Test 0d: sample from StateSampler")
gymEnv = gym.make("Taxi-v3")
gymEnv.reset()
stateSampler = pySims.GymResetSampler(gymEnv)
print(stateSampler.sample(10))
assert stateSampler.sample(10).shape == (10,1)
print("Test 0d is OK")


#####Test 1: try to reset the state0 from given state0
print("Start Test 1d: ResetFromState0")
gymEnv = gym.make("Taxi-v3")
gymEnv.reset()
state0 = np.array([25])
sim=pySims.GymSimulator(gymEnv)
...
#####Test 2: try to reset the state0 from stateSampler and check if the dimensions are correct
print("Start Test 2d: ResetFromStateSampler")
gymEnv = gym.make("Taxi-v3")
gymEnv.reset()
sim=pySims.GymSimulator(gymEnv)
stateSampler = pySims.GymResetSampler(gymEnv)
...
#####Test 3: Check if transitionSampler works correctly for one s0
print("Start Test 3d: SampleTransitionFromState0")
gymEnv = gym.make("Taxi-v3")
state0 = np.concatenate([np.array([[31]]) for k in np.arange(5)],axis=0)
gymEnv.reset()
sim=pySims.GymSimulator(gymEnv)
print('DEBUG TESTS ActionSpaceShape',gymEnv.action_space.shape)
def policy(s):
...
print('Test Sample')
print(sample)
assert sample[0].shape == (5,1,1) and sample[1].shape == (5,1,1) and sample[2].shape == (5,1,1)
print("Test 3d is OK")


#####Test 4: Check if transitionSampler works correctly for stateSampler
print("Start Test 4d: SampleTransitionFromStateSampler")
gymEnv = gym.make("Taxi-v3")
gymEnv.reset()
sim=pySims.GymSimulator(gymEnv)
stateSampler = pySims.GymResetSampler(gymEnv)
...
print("Start Test 5d: Sample Trajectories From S0s")
gymEnv = gym.make("Taxi-v3")
print('DEBUG',gymEnv.reset())
state0 = np.concatenate([np.array([gymEnv.reset()])[None,:] for k in np.arange(5)],axis=0)
gymEnv.reset()
sim=pySims.GymSimulator(gymEnv)
def policy(s):
    #returns action based on state
...
print('DEBUG: actionsShape',sample[1].shape)
print('DEBUG: rewardsShape',sample[2].shape)
assert sample[0].shape[0] == 5 and sample[0].shape[1] == 1
assert sample[1].shape[0] == 5 and sample[1].shape[1] == 1
assert sample[2].shape[0] == 5 and sample[2].shape[1] == 1
print("Test 5d is OK")


print("Start Test 6d: Sample Trajectories From StateSampler")
gymEnv = gym.make("Taxi-v3")
print('DEBUG',gymEnv.reset())
gymEnv.reset()
sim=pySims.GymSimulator(gymEnv)
...
print("Start Test 7d: Sample Trajectories From S0s (Parallel)")
gymEnv = gym.make("Taxi-v3")
print('DEBUG',gymEnv.reset())
state0 = np.concatenate([np.array([gymEnv.reset()])[None,:] for k in np.arange(5)],axis=0)
gymEnv.reset()
...
print("Start Test 8d: Sample Trajectories From StateSampler (Parallel)")
gymEnv = gym.make("Taxi-v3")
print('DEBUG',gymEnv.reset())
gymEnv.reset()
sim=pySims.GymSimulator(gymEnv)
